...
class ProductMeta {
    ...
    public function __construct() {
        add_action( 'woocommerce_product_options_pricing', [ $this, 'add_samrprca_buying_price_field' ] );
        add_action( 'woocommerce_admin_process_product_object', [ $this, 'save_samrprca_buying_price_field' ] );
        add_action( 'woocommerce_checkout_create_order_line_item', [ $this, 'add_samrprca_buying_price_to_order_item' ], 10, 4 );

        add_action( 'woocommerce_variation_options_pricing', [ $this, 'add_variation_samrprca_buying_price_field' ], 10, 3 );
        add_action( 'woocommerce_save_product_variation', [ $this, 'save_variation_samrprca_buying_price_field' ], 10, 2 );
    }
     ...
     */
    public function add_samrprca_buying_price_field() {
        global $post;
        ...
        if ( $description ) {
            echo '<p class="form-field _samrprca_samrprca_buying_price_last_updated_field">';
     ...
     */
    public function save_samrprca_buying_price_field( $product ) {
        // Nonce validation
        ...
        if ( isset( $_POST['_samrprca_buying_price'] ) ) {
            $buying_price = sanitize_text_field( wp_unslash( $_POST['_samrprca_buying_price'] ) );
            ...
            $product->update_meta_data( '_samrprca_buying_price', $buying_price );
            $product->update_meta_data( '_samrprca_buying_price_last_updated', current_time( 'mysql' ) );
        }
    }
    
    public function add_variation_samrprca_buying_price_field( $loop, $variation_data, $variation ) {
        $product = wc_get_product( $variation->ID );
        $buying_price = $product->get_meta( '_samrprca_buying_price' );

        woocommerce_wp_text_input(
            [
                'id'            => "_variation_samrprca_buying_price_{$loop}",
                'name'          => "_variation_samrprca_buying_price[{$loop}]",
                'value'         => $buying_price,
                'label'         => __( 'Buying Price', 'samrat-profit-calculator-for-woocommerce' ) . ' (' . get_woocommerce_currency_symbol() . ')',
                'desc_tip'      => true,
                'description'   => __( 'Enter the buying price for this variation.', 'samrat-profit-calculator-for-woocommerce' ),
                'type'          => 'number',
    ...
    public function save_variation_samrprca_buying_price_field( $variation_id, $i ) {
        ...
        if ( isset( $_POST['_variation_samrprca_buying_price'][ $i ] ) ) {
            $buying_price = sanitize_text_field( wp_unslash( $_POST['_variation_samrprca_buying_price'][ $i ] ) );
